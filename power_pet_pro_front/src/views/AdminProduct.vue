            ...
            <form class="box" @submit.prevent="submit_product()">
              ...
              <div id="file-js-example" class="file has-name is-centered">
                <label class="file-label">
                  ...
                  <span>
                    <button class="button is-danger is-outlined" @click.prevent="remove_file()">
...
export default{
  ...
  methods:{
    ...
    submit_product(){
      if(this.chosen_category && this.product_name && this.product_price && this.product_description){
        // We need to make sure we get the chosen_category id so
        console.log(this.product_image)
        let category_object = this.store_categories.find(category => category.name === this.chosen_category)
