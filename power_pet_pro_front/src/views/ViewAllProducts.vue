<template>
    ...
    <tbody>
     <tr v-for="(product, index) in products" :key="index">
       <th>
        ...
        <th>
          <button class="button is-warning is-ghost" @click="edit_mode_activated(product)">
            <i class="fas fa-pencil-alt"></i>
          </button>&nbsp;
          <button class="button is-danger is-ghost" @click="delete_mode_activated(product)">
  ...
  <nav class="pagination is-centered" role="navigation" aria-label="pagination">
    <button class="pagination-previous" @click="changeView(previous)">Previous</button>
  ...
  <div class="modal" :class="{'is-active':edit_mode}">
    ...
    <div class="modal-card">
      <header class="modal-card-head">
        ...
        <button class="delete" aria-label="close" @click="edit_mode = !edit_mode"></button>
      </header>
  ...
  <div class="modal" :class="{'is-active':delete_mode}">
    ...
    <div class="modal-card">
      <header class="modal-card-head">
        ...
        <button class="delete" aria-label="close" @click="delete_mode = !delete_mode"></button>
      </header>
      ...
      <footer class="modal-card-foot">
        <button class="button is-danger" @click="delete_product()">Delete Product</button>
        <button class="button" @click="delete_mode = !delete_mode">Cancel</button>
      </footer>
    </div>
  </div>
</template>
...
export default{
  ...
  methods:{
    ...
    changeView(url){
      if(url !== null){
        axios.get(url, {headers: {Authorization: `Bearer ${Cookies('accessToken')}`}}).then(response=>{
    ...
    delete_mode_activated(product){
      ...
      this.chosen_product_description = product.description
      this.chosen_product_price = product.price
      this.chosen_product_productUrl = this.productUrl(product.get_absolute_url)
      console.log(this.chosen_product_productUrl)
    },
    delete_product(){
      axios.delete(`admin_panel/product_list/update/${this.chosen_product_id}/`,{
  ...
  created(){
    axios.get('product_list/', {headers: {Authorization: `Bearer ${Cookies('accessToken')}`}} ).then((response)=>{
      this.products = response.data.results
      this.next = response.data.next
      this.previous = response.data.previous
      this.totalProductCount = response.data.count
