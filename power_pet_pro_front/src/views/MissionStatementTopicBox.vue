<template>
    ...
    <div class="hero-body">
      <div class="container has-text-centered">
        ...
        <div class="columns is-centered">
          ...
          <!-- Here we are going to the box to display the messages -->
           <div class="column is-5">
             <div class="box has-text-centered">
               <h1 class="title is-3">
                 Your Current Mission Statement Topics:
               </h1>
               <div v-if="mission_statement_topics">
                 <ul>
                   <li v-for="(topic, index) in mission_statement_topics" :key="index">
                     {{ index + 1 }}. {{ topic.topic }}
                      <button class="button is-warning is-light is-small" @click="edit_mission_topic_activation(topic)">
                        <span>Edit Topic</span>
                        <span class="icon is-small">
                          <i class="fas fa-pencil-alt"></i>
                        </span>
                      </button>&nbsp;
                      <button class="button is-danger is-light is-small" @click="delete_mission_topic_activation(topic)">
          ...
          <div class="column is-5">
            <form class="box" @submit.prevent="activate_modal = !activate_modal">
              <figure class="image is-128x128 is-inline-block">
               ...
               <div class="field">
                  <div class="control has-icons-left has-icons-right">
                    <input class="input is-medium" type="text" placeholder="Add a Topic" v-model="new_topic">
              ...
              <button class="button is-success">Submit Mission Statement</button>
            </form>
          </div>
  ...
  <div class="modal is-centered" :class="{'is-active':activate_modal, 'is-clipped':activate_modal}">
    ...
    <div class="modal-card">
      ...
      <section class="modal-card-body">
        <div class="box">
          New Mission Topic: "{{ new_topic }}"
        </div>
      </section>
      <footer class="modal-card-foot is-centered">
        <button class="button is-success" @click="submit_mission_topic()">Submit</button>
  ...
  <div class="modal is-centered" :class="{'is-active':activate_edit_modal, 'is-clipped':activate_edit_modal}">
    ...
    <div class="modal-card">
      ...
      <footer class="modal-card-foot is-centered">
        <button class="button is-success" @click="edit_mission_topic()">Save Changes</button>
        <button class="button" @click="activate_modal=false">Cancel</button>
      </footer>
    </div>
  </div>
   ...
   <div class="modal is-centered" :class="{'is-active':activate_delete_modal, 'is-clipped':activate_delete_modal}">
    ...
    <div class="modal-card">
      <header class="modal-card-head">
        ...
        <button class="delete" aria-label="close" @click="activate_delete_modal=false"></button>
      </header>
      <section class="modal-card-body">
        <div class="box">
          Current Mission Topic: "{{ delete_topic }}"
        </div>
      </section>
      <footer class="modal-card-foot is-centered">
        <button class="button is-danger" @click="delete_mission_topic">Delete Message</button>
        <button class="button" @click="activate_delete_modal=false">Cancel</button>
      </footer>
    </div>
  </div>
</template>
  ...
  methods:{
    submit_mission_topic(){
      if(this.new_topic){
        ...
        axios.post('admin_panel/our_mission/add_topic/',{
          ...
          toast({
            message: "Submitted the new mission topic. Please refresh your page.",
            type: "is-success",
            dismissible: true,
            pauseOnHover: true,
    ...
    delete_mission_topic(){
      axios.delete(`admin_panel/our_mission/update_topic/${this.delete_topic_slug}/`,{
        headers: {Authorization: `Bearer ${this.accessToken}`}
      }).then((response)=>{
    ...
    edit_mission_topic(){
      ...
      axios.put(`admin_panel/our_mission/update_topic/${this.edit_topic_slug}/`,{
